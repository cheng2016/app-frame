    ...
    init {
        viewModelScope.launch {
            when (val result = userRepository.profile()) {
                is AppResult.Success -> _nickname.value = result.data.nickname
...
@AndroidEntryPoint
class UserDetailFragment : Fragment(R.layout.fragment_simple_form) {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.fieldOneLayout.hint = "昵称"
        binding.fieldTwoLayout.isVisible = false
        binding.submitButton.text = "保存"
        binding.submitButton.setOnClickListener {
            viewModel.save(binding.fieldOne.text?.toString().orEmpty())
        }
        viewLifecycleOwner.lifecycleScope.launch {
            viewLifecycleOwner.repeatOnLifecycle(Lifecycle.State.STARTED) {
                launch {
                    viewModel.nickname.collect {
                        if (binding.fieldOne.text.isNullOrEmpty()) binding.fieldOne.setText(it)
                    }
                }
